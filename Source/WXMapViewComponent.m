...
@implementation WXMapViewComponent
{
    ...
    BOOL _showCompass;
    BOOL _rotateEnabled;
    BOOL _skyModelEnable;
    BOOL _rotateCameraEnabled;
    NSString *_userTrackingMode;
}
               ...
               weexInstance:(WXSDKInstance *)weexInstance
{
    ...
    if (self) {
        NSArray *center = [attributes wxmap_safeObjectForKey:@"zxeCenter"];
        if ([WXConvert isValidatedArray:center]) {
            _centerCoordinate = [WXConvert CLLocationCoordinate2D:center];
        }
        NSLog(@"center === %f, %f", _centerCoordinate.longitude, _centerCoordinate.latitude);
        _zoomLevel = [[attributes wxmap_safeObjectForKey:@"zoom"] floatValue];
        _showScale = [[attributes wxmap_safeObjectForKey:@"scale"] stringToBool];
        _showGeolocation = [[attributes wxmap_safeObjectForKey:@"geolocation"] stringToBool];
        _showCompass = [[attributes wxmap_safeObjectForKey:@"compass"] stringToBool];
        _rotateEnabled = [[attributes wxmap_safeObjectForKey:@"rotate"] stringToBool];
        _showUserLocation = [[attributes wxmap_safeObjectForKey:@"showUser"] stringToBool];
        _userTrackingMode = [attributes wxmap_safeObjectForKey:@"trackingMode"];
        _skyModelEnable = [[attributes wxmap_safeObjectForKey:@"skyModelEnable"] stringToBool];
        _rotateCameraEnabled = [[attributes wxmap_safeObjectForKey:@"rotateCameraEnabled"] stringToBool];
...
- (void)viewDidLoad
{
    ...
    self.mapView.showsCompass = _showCompass;
    self.mapView.rotateEnabled = _rotateEnabled;
//    self.mapView.skyModelEnable = _skyModelEnable;
    self.mapView.rotateCameraEnabled = _rotateCameraEnabled;
    if (_centerCoordinate.latitude != 0 && _centerCoordinate.longitude != 0) {
        [self.mapView setCenterCoordinate:_centerCoordinate];
    } else {
        self.mapView.showsUserLocation = _showUserLocation;
    }

    self.mapView.userTrackingMode = [_userTrackingMode isEqualToString:@"follow"] ? MAUserTrackingModeFollow : [_userTrackingMode isEqualToString:@"followHeader"] ? MAUserTrackingModeFollowWithHeading : MAUserTrackingModeNone;
    [self.mapView setZoomLevel:_zoomLevel];
    
    MAUserLocationRepresentation *location = [[MAUserLocationRepresentation alloc] init];
    location.showsAccuracyRing = NO;
    location.showsHeadingIndicator = NO;
    location.enablePulseAnnimation = NO;
    location.image = [UIImage imageNamed:@"navi_map_gps_locked"];
...
{
    if ([WXConvert isValidatedArray:attributes[@"zxeCenter"]]) {
        [self setCenter:attributes[@"zxeCenter"]];
    }
...
- (void)mapView:(MAMapView *)mapView didDeselectAnnotationView:(MAAnnotationView *)view
{
    ...
    for (WXComponent *component in self.subcomponents) {
        if ([component isKindOfClass:[WXMapViewMarkerComponent class]] &&
            [component.ref isEqualToString:annotation.component.ref]) {
            WXMapViewMarkerComponent *marker = (WXMapViewMarkerComponent *)component;
            if (marker.unClickEvent) {
                [marker fireEvent:marker.unClickEvent params:[NSDictionary dictionary]];
            }
        }
    }
}
...
- (void)mapView:(MAMapView *)mapView mapDidZoomByUser:(BOOL)wasUserAction {
    if (_zoomChanged) {
        [self fireEvent:@"zoomchange" params:@{@"zoom": @(mapView.zoomLevel)}];
    }
}

//- (void)mapView:(MAMapView *)mapView regionDidChangeAnimated:(BOOL)animated {
//    if (_regionChanged) {
//        [self fireEvent:@"regionchange"
//                params: @{
...
{
    if (_isDragend) {
        [self fireEvent:@"dragend" params:@{@"region": @{
                                                @"center": @{
                                                    @"latitude": @(mapView.region.center.latitude),
                                                    @"longitude": @(mapView.region.center.longitude)
                                                },
...
- (MAOverlayRenderer *)mapView:(MAMapView *)mapView rendererForOverlay:(id <MAOverlay>)overlay
{
    if ([overlay isKindOfClass:[MAPolyline class]])
    {
        ...
        polylineRenderer.lineDashType = [WXConvert isLineDash:component.strokeStyle];
    ...
    }else if ([overlay isKindOfClass:[MAPolygon class]])
    {
        ...
        polygonRenderer.lineDashType = [WXConvert isLineDash:component.strokeStyle];
